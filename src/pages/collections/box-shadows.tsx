...
const JSON_LD_SCHEMAS = [
	{
		...
		author: { '@type': 'Organization', name: 'UIXplor', url: 'https://uixplor.com' },
	},
	{
		'@context': 'https://schema.org',
		'@type': 'BreadcrumbList',
		itemListElement: [
			...
			{ '@type': 'ListItem', position: 3, name: 'CSS Box Shadows', item: 'https://uixplor.com/collections/box-shadows' },
		],
	},
	{
		'@context': 'https://schema.org',
...
export default function BoxShadows() {
	...
	const filtered = useMemo(() => {
		let result = activeCategory === 'all' ? shadows : shadows.filter((s) => s.category === activeCategory);
		if (search.trim()) {
			...
			<main className="min-h-screen px-4 sm:px-6 py-8 sm:py-12">
				<div className="max-w-7xl mx-auto">
					...
					<nav aria-label="Breadcrumb" className="mb-8">
						<ol className="flex items-center gap-2 text-sm text-white/40" itemScope itemType="https://schema.org/BreadcrumbList">
							<li itemProp="itemListElement" itemScope itemType="https://schema.org/ListItem">
								<Link href="/" className="hover:text-[#B8FB3C] transition-colors" itemProp="item">
							...
							<li itemProp="itemListElement" itemScope itemType="https://schema.org/ListItem">
								<Link href="/collections" className="hover:text-[#B8FB3C] transition-colors" itemProp="item">
					...
					>
						...
						<p className="text-base sm:text-lg text-white/60 max-w-2xl mx-auto mb-4">
							{shadows.length} handcrafted CSS box shadow presets — soft, layered, neumorphic, glow, colored, glassmorphism, brutalist & more.
							Click any card to view and copy the code instantly.
						</p>
					...
					<section aria-label="Introduction" className="mb-10 max-w-3xl mx-auto text-center">
						<p className="text-sm sm:text-base text-white/45 leading-relaxed">
							Looking for <strong className="text-white/70">CSS box shadow copy and paste</strong> code? You&apos;ve found the right place.
							UIXplor&apos;s free shadow library gives you {shadows.length} production-ready{' '}
							<strong className="text-white/70">CSS box shadow examples</strong> across every style —
							from ultra-subtle soft shadows and multi-layer depth effects to bold neumorphic and colored glow shadows.
							Each preset is tested across Chrome, Firefox, and Safari. No sign-up, no subscription —
							open a card, click copy, and paste the CSS directly into your project. Compatible with
							plain CSS, SCSS, styled-components, and{' '}
							<strong className="text-white/70">Tailwind CSS</strong> arbitrary value syntax.
						</p>
					</section>
					...
					{/* Category Filters */}
					<div className="flex flex-wrap gap-2 mb-10 justify-center" role="group" aria-label="Filter shadows by category">
						{CATEGORIES.map((cat) => {
						...
						<AnimatePresence mode="popLayout">
							{filtered.map((shadow, index) => {
								...
								return (
									...
									>
										{/* Shadow preview */}
										<div className="p-6 flex items-center justify-center h-32 sm:h-36 bg-white rounded-t-2xl" aria-hidden="true">
											<div
					...
					{filtered.length === 0 && (
						<div className="text-center py-20" role="status">
							...
							{search && <button onClick={() => setSearch('')} className="text-[#B8FB3C] text-sm underline">Clear search</button>}
						</div>
					)}
					...
					<article className="mt-20 max-w-3xl mx-auto prose-invert space-y-10">
						...
						<section>
							...
							<p className="text-white/50 leading-relaxed text-sm sm:text-base">
								The CSS <code className="text-[#B8FB3C] bg-[#B8FB3C]/10 px-1.5 py-0.5 rounded text-xs">box-shadow</code> property
								adds one or more shadow effects around an element&apos;s frame. The full syntax is:
							</p>
							<pre className="mt-3 mb-4 bg-white/4 border border-white/8 rounded-xl p-4 text-xs sm:text-sm text-white/70 overflow-x-auto leading-relaxed">
								<code>box-shadow: offset-x offset-y blur-radius spread-radius color;</code>
							</pre>
							<p className="text-white/50 leading-relaxed text-sm sm:text-base">
								You can layer multiple shadows by separating them with commas — a powerful technique for creating
								realistic depth and multi-colored glow effects. Adding the <code className="text-[#B8FB3C] bg-[#B8FB3C]/10 px-1.5 py-0.5 rounded text-xs">inset</code> keyword
								makes the shadow appear inside the element rather than outside, useful for pressed-button states and
								sunken inputs. Every shadow in UIXplor&apos;s library is a real, tested{' '}
								<strong className="text-white/70">CSS box shadow example</strong> you can copy and paste directly into your project.
							</p>
						</section>
						...
						<section>
							...
							<p className="text-white/50 leading-relaxed text-sm sm:text-base mb-6">
								This free <strong className="text-white/70">CSS shadow preset</strong> library is categorised by visual style
								so you can quickly find the right shadow for your design system.
							</p>

							<div className="space-y-5">
								<div>
									<h3 className="text-base sm:text-lg font-semibold text-white mb-1">🌫️ Soft Shadows</h3>
									<p className="text-white/50 text-sm leading-relaxed">
										Barely-there shadows that add a whisper of depth without visual weight. Perfect for card UIs,
										minimalist dashboards, and product landing pages. These shadows use very low opacity values
										and wide blur radii to create the illusion of floating elements.
									</p>
								</div>
								<div>
									<h3 className="text-base sm:text-lg font-semibold text-white mb-1">📦 Layered / Stacked Shadows</h3>
									<p className="text-white/50 text-sm leading-relaxed">
										Multi-layer shadows that combine 3–4 separate shadow values at increasing distances.
										The result looks far more natural and realistic than a single box-shadow because it
										mimics how light scatters rather than casting a uniform shadow.
									</p>
								</div>
								<div>
									<h3 className="text-base sm:text-lg font-semibold text-white mb-1">🪨 Deep Elevation Shadows</h3>
									<p className="text-white/50 text-sm leading-relaxed">
										Bold, high-spread shadows that lift elements dramatically off the background —
										ideal for modals, floating action buttons, tooltips, and hero sections where you
										need a strong Material Design-style elevation.
									</p>
								</div>
								<div>
									<h3 className="text-base sm:text-lg font-semibold text-white mb-1">🧱 Neumorphic Shadows</h3>
									<p className="text-white/50 text-sm leading-relaxed">
										The viral Neumorphism design trend uses dual shadows — a dark shadow on one side and a bright
										highlight on the other — to simulate elements extruded from a surface. These{' '}
										<strong className="text-white/70">neumorphic CSS box shadow</strong> presets are ready to drop
										into your soft-UI projects.
									</p>
								</div>
								<div>
									<h3 className="text-base sm:text-lg font-semibold text-white mb-1">✨ Glow Shadows</h3>
									<p className="text-white/50 text-sm leading-relaxed">
										Colorized, spread-heavy shadows that radiate light from behind an element. Widely used in
										dark-mode UIs, gaming dashboards, and cyberpunk-style designs. UIXplor includes both
										monochrome and tinted glow variants.
									</p>
								</div>
								<div>
									<h3 className="text-base sm:text-lg font-semibold text-white mb-1">🔲 Inset Shadows</h3>
									<p className="text-white/50 text-sm leading-relaxed">
										Inner shadows that make an element appear sunken into the page. Applications include
										pressed button states, text input fields, progress bars, and carved-out containers.
										Inset shadows are defined by prepending the{' '}
										<code className="text-[#B8FB3C] bg-[#B8FB3C]/10 px-1.5 py-0.5 rounded text-xs">inset</code> keyword.
									</p>
								</div>
							</div>
						</section>
						...
						<section>
							...
							<ol className="space-y-3 text-white/50 text-sm leading-relaxed list-none">
								<li className="flex gap-3">
									...
									<span>Browse the shadow grid above and click <strong className="text-white/70">View Code →</strong> on any card.</span>
								</li>
								<li className="flex gap-3">
									<span className="text-[#B8FB3C] font-bold shrink-0">02.</span>
									<span>Copy the CSS tab — it contains the full <code className="text-[#B8FB3C] bg-[#B8FB3C]/10 px-1.5 py-0.5 rounded text-xs">box-shadow</code> value inside a ready-to-use class.</span>
								</li>
								<li className="flex gap-3">
									<span className="text-[#B8FB3C] font-bold shrink-0">03.</span>
									<span>Paste into your CSS file, Tailwind config, or CSS-in-JS theme object.</span>
								</li>
								<li className="flex gap-3">
									<span className="text-[#B8FB3C] font-bold shrink-0">04.</span>
									<span>Adjust the color, blur, or spread to match your design tokens.</span>
								</li>
							</ol>
						</section>
						...
						<section>
							...
							<p className="text-white/50 leading-relaxed text-sm sm:text-base mb-4">
								If you use <strong className="text-white/70">Tailwind CSS</strong>, the framework ships with built-in shadow utilities that map directly to CSS box shadows.
								The standard scale is <code className="text-[#B8FB3C] bg-[#B8FB3C]/10 px-1.5 py-0.5 rounded text-xs">shadow-sm</code>,{' '}
								<code className="text-[#B8FB3C] bg-[#B8FB3C]/10 px-1.5 py-0.5 rounded text-xs">shadow</code>,{' '}
								<code className="text-[#B8FB3C] bg-[#B8FB3C]/10 px-1.5 py-0.5 rounded text-xs">shadow-md</code>,{' '}
								<code className="text-[#B8FB3C] bg-[#B8FB3C]/10 px-1.5 py-0.5 rounded text-xs">shadow-lg</code>,{' '}
								<code className="text-[#B8FB3C] bg-[#B8FB3C]/10 px-1.5 py-0.5 rounded text-xs">shadow-xl</code>, and{' '}
								<code className="text-[#B8FB3C] bg-[#B8FB3C]/10 px-1.5 py-0.5 rounded text-xs">shadow-2xl</code>.
							...
							<p className="text-white/50 leading-relaxed text-sm sm:text-base">
								To add the UIXplor presets to your Tailwind design system, extend the{' '}
								<code className="text-[#B8FB3C] bg-[#B8FB3C]/10 px-1.5 py-0.5 rounded text-xs">theme.boxShadow</code> key in{' '}
								<code className="text-[#B8FB3C] bg-[#B8FB3C]/10 px-1.5 py-0.5 rounded text-xs">tailwind.config.js</code>:
							</p>
      ...
      boxShadow: {
        ...
        'glow':  '0 0 16px rgba(100,120,200,0.15), 0 0 48px rgba(100,120,200,0.08)',
						...
						<section>
							...
							<p className="text-white/50 leading-relaxed text-sm sm:text-base mb-4">
								A common source of confusion: when should you use CSS <code className="text-[#B8FB3C] bg-[#B8FB3C]/10 px-1.5 py-0.5 rounded text-xs">box-shadow</code> vs.{' '}
								<code className="text-[#B8FB3C] bg-[#B8FB3C]/10 px-1.5 py-0.5 rounded text-xs">filter: drop-shadow()</code>?
							</p>
							<div className="overflow-x-auto">
								<table className="w-full text-xs sm:text-sm text-left border-collapse">
									<thead>
										<tr className="border-b border-white/10">
											<th className="px-3 py-2 text-white/70 font-semibold">Feature</th>
											<th className="px-3 py-2 text-white/70 font-semibold">box-shadow</th>
											<th className="px-3 py-2 text-white/70 font-semibold">filter: drop-shadow()</th>
										</tr>
									</thead>
									<tbody className="text-white/45">
										...
										<tr className="border-b border-white/6">
											<td className="px-3 py-2">Works on transparent PNGs / SVGs</td>
							...
							<p className="text-white/50 leading-relaxed text-sm sm:text-base mt-4">
								Use <code className="text-[#B8FB3C] bg-[#B8FB3C]/10 px-1.5 py-0.5 rounded text-xs">box-shadow</code> for standard rectangular HTML elements.
								Use <code className="text-[#B8FB3C] bg-[#B8FB3C]/10 px-1.5 py-0.5 rounded text-xs">filter: drop-shadow()</code> when you need a shadow that hugs the silhouette of an icon,
							...
							<dl className="space-y-5">
								{[
									...
									<div key={q} className="border border-white/8 rounded-xl p-5">
										<dt className="text-sm sm:text-base font-semibold text-white mb-2">{q}</dt>
										<dd className="text-sm text-white/50 leading-relaxed">{a}</dd>
									</div>
								))}
							</dl>
						</section>
						...
						<section>
							...
							<div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
								...
								>
									...
									<span className="text-sm font-semibold text-white group-hover:text-[#B8FB3C] transition-colors">Gradient Generator</span>
									<span className="text-xs text-white/40">CSS gradient presets — copy &amp; paste</span>
								</Link>
								<Link
								...
								>
									<span className="text-[#B8FB3C] text-lg">🪟</span>
									<span className="text-sm font-semibold text-white group-hover:text-[#B8FB3C] transition-colors">Glass Effect Generator</span>
									<span className="text-xs text-white/40">Glassmorphism CSS snippets</span>
								</Link>
								<Link
									href="/collections"
									className="group flex flex-col gap-1 p-4 rounded-xl border border-white/8 hover:border-[#B8FB3C]/30 hover:bg-[#B8FB3C]/5 transition-all duration-300"
								>
									...
									<span className="text-sm font-semibold text-white group-hover:text-[#B8FB3C] transition-colors">CSS Tools Library</span>
									<span className="text-xs text-white/40">All free CSS generators &amp; snippets</span>
								</Link>
							</div>
						</section>
