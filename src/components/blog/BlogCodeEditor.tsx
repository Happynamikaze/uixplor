...
export default function BlogCodeEditor({ html, css, title = 'Try the Code' }: BlogCodeEditorProps) {
...
${html}
  ...
  return (
    ...
    >
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2">
            <svg width="14" height="14" fill="none" stroke="currentColor" className="opacity-50" viewBox="0 0 24 24" style={{ color: '#B8FB3C' }}>
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 20l4-16m4 4l4 4-4 4M6 16l-4-4 4-4" />
            </svg>
            <span className="text-xs font-semibold" style={{ color: 'rgba(255,255,255,0.6)' }}>{title}</span>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <span className="text-[10px] font-semibold px-2 py-1 rounded-md" style={{ background: 'rgba(184,251,60,0.08)', color: '#B8FB3C', border: '1px solid rgba(184,251,60,0.2)' }}>
            Live Editor
          </span>
          <button
            onClick={() => {
            ...
            style={{ background: copied ? 'rgba(184,251,60,0.15)' : 'rgba(255,255,255,0.06)', color: copied ? '#B8FB3C' : 'rgba(255,255,255,0.5)', border: `1px solid ${copied ? 'rgba(184,251,60,0.3)' : '#2A2A2A'}` }}
          >
      ...
      >
        <SandpackLayout style={{ borderRadius: 0, border: 'none', minHeight: '320px' }}>
          <SandpackCodeEditor
            showLineNumbers
            showInlineErrors
            wrapContent
      ...
      <div
        ...
        style={{ background: '#08080f', borderColor: '#2A2A2A', color: 'rgba(255,255,255,0.2)' }}
      >
